...
function CliReporter(emitter, stream, options) {
  ...
  emitter.on('browser-init', function(browser) {
    ...
    this.browserStates[browser.id]  = 'initializing';
    this.updateStatus();
  }.bind(this));
  ...
  emitter.on('browser-start', function(browser, data, stats) {
    ...
    this.browserStates[browser.id] = 'running';
    this.updateStatus();
  }.bind(this));
  ...
  emitter.on('browser-end', function(browser, error) {
    if (error) {
      ...
      this.browserStates[browser.id] = chalk.red('failed');
    } else {
...
CliReporter.prototype.updateStatus = function() {
  ...
  var statuses = Object.keys(this.browserStates).map(function(browserId) {
    var pretty = this.prettyBrowsers[browserId];
    var stats  = this.browserStats[browserId];
    var status = this.browserStates[browserId];
